...
| BUG-0075 | TB_BUG | CLOSED | 共享译码 oracle `decode_mst_port` 未实现 spec §3.2.3 `end_addr=='0` 哨兵语义——M6 处置选 option 2（env 约束路线）：§3.2 clause 4 追加环境约束使哨兵构造性不可触发，oracle 维持现状，rev 评审 PASS | 静态（spec §3.2 clause 4 env 约束在位 + 四表 end_addr 非零） | doc/bugs/BUG-0075.md |
