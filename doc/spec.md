...
3. **终址哨兵 `end_addr == '0`（来源：RTL——上游文档未载）**：`end_addr` 全零
   不表示零长度区间，而是编码「区间延伸至地址空间末端」，等效终址为
   `2^AxiAddrWidth`（开区间上限在 `AxiAddrWidth` 位宽下溢出截断为全零的结果）。
   下文记
   `effective_end = (end_addr == '0) ? 2^AxiAddrWidth : end_addr`。
   故 clause 1 的匹配判据在该编码下退化为 `addr >= start_addr`（上界恒真）。
4. **实现侧附加约束：区间非空（来源：RTL——上游文档未载）**：clause 2 的
   `start_addr <= end_addr` 取自权威文档，是本 spec 的许可口径，不因本条改动；
   实现侧另要求 `start_addr < effective_end`，即禁止空区间。二者仅在
   `start_addr == end_addr` 一点上有分歧，且该分歧受 clause 3 的哨兵调制：
   `start_addr == end_addr == '0` 合法（哨兵命中，该 rule 覆盖整个地址空间）；
...
3. decode error slave 吞下（absorb）整个事务，并以 decode error 应答
   ...
   **beat 数正确**（xbar.md "proper number of beats" 涵盖读/写两路，C5
   ...
   （DECERR）。
4. 读响应每个 beat 的数据 = err_slv 的默认参数 `RespData =
   64'hCA11AB1EBADCAB1E`（`RespWidth=64`；`axi_xbar_unmuxed.sv:195/238` 两处
   例化 `axi_err_slv` 均**未覆写** `RespData`/`RespWidth`，故取此默认），
   SV 赋值 `r.data = RespData` 按 `AxiDataWidth` **零扩展**（更宽时高位补 0）
   **或截断**（更窄时取低 `AxiDataWidth` 位）。基线 `AxiDataWidth=64` ⇒
   `r.data = 64'hCA11AB1EBADCAB1E`（下 32 位 `0xBADCAB1E`、上 32 位
   `0xCA11AB1E`）；`AxiDataWidth=32` ⇒ 截为 `32'hBADCAB1E`。**（来源：
   RTL——`axi_err_slv.sv:24-25`、`axi_xbar_unmuxed.sv:195/238`；上游文档
   xbar.md L33 把基数记作 `32'hBADCAB1E`、上 32 位零扩展为 0，与 err_slv
   实际默认 `RespData` 高半 `0xCA11AB1E` 不符——L33 仅在 `AxiDataWidth=32`
   截断情形下恰好正确。BUG-0033 裁决，REV-014。）**
...
7. **ATOP × 译码未命中的应答形态，许可来源未定义**：一笔 `aw.atop != '0`
   ...
   master port 使能位/索引运行时可变，若窄读为"仅未使能 default 时的未命中
   地址"，一次运行时关闭 default port（M4-RC01 场景）会使同一地址的宽窄
   归属瞬时反转，把移动靶引入约束；其二，窄读法唯一解锁的路径（ATOP 经
   default port 抵达真实 master 端口）之功能语义已由命中地址场景覆盖
   （SPEC-6.3/6.4，M2-AT01/M3-AT02），维持宽读不解任何缺口、零额外覆盖收益。
