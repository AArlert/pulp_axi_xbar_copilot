...
- **C2.3** 读响应每 beat 数据期望 = `32'hBADCAB1E`，按 `AxiDataWidth` 零扩展/截断。
  依据：spec §4.4。
- **C2.4（M3 激活时补足）** 归因与分流：每个 slave 端口有**自己的** decode error
  slave，故一笔未命中事务的 DECERR 应答期望归因到**其发起端口自身**，不得被记成
  其它端口的响应（依据 spec §4.1）。当该端口 `en_default_mst_port_i[i]=1` 时，未
  命中事务的期望目标改为 `default_mst_port_i[i]` 所指的**真实 master 端口**，其
  响应码由该端口的 slave agent 决定（正常应答，非 DECERR），且 master 侧 ID 前缀/
  响应回送判据（§3）照常适用。依据：spec §3.3、§4.2。
- **C2.5（M3 激活）** ATOP 与未命中路径的组合**不在期望值范围内**：env 保证不向
  译码未命中地址发起 ATOP（uvm_env C6.2 / BUG-0032），故参考模型**不得**为该组合
  编造期望——若观测到该组合发生，应报 TB_BUG（env 违反自身约束），而非按任一读法
  判 DUT。依据：spec §0 行 6。
...
- **C5.6（M3 新增，译码未命中事务的保序地位；依据 spec §5.2.6）** 完成序判据的
  适用范围分三层，缺一层即为错：
  - **default master port 半边**：走 default port 的事务其目标是**真实 master
    端口**，C5.1/C5.2 与 §5.2 第 1-4 条**原样适用**，与命中 rule 的事务无区别
    （spec §5.2.6 第 1 条）。
  - **完整 ID 维度（必须判决）**：同一 slave 端口上**完整 ID 相同**、同方向的
    事务，其 B/rlast 完成序须与接受序一致——**无论**该事务被路由到 master 端口、
    default master port 还是内部 err_slv。参考模型**必须**把译码未命中事务纳入
    这一维度的队列判据（spec §5.2.6 第 2.a 条）。
  - **低位 ID 桶维度（不得判决）**：仅低 `AxiIdUsedSlvPorts` 位相同、**完整 ID
    不同**、且其中一笔走 err_slv 时，二者完成序关系许可来源未定义，**不得**写
    判决；排除须在代码中以**引 spec §5.2.6 的注释显式写出**，并配一条非判决
    cover 留痕（`functional_coverage.md` §5）。**红线（spec §5.2.6 第 3 条）**：
    不得以"未登记 ⇒ 目标/序号读到默认或陈旧值 ⇒ 比较恰好为假"实现该排除——那
    在报告上与"忘了写"完全同形，且陈旧值会继续参与比较、可产生无来源假红。
  - 激励来源：M3-DE02（第 1 层）、M3-OR04（第 2/3 层）。
- **C5.7（M3 新增，配置点相关）** ID 前缀宽度按当前配置点的 `$clog2(NoSlvPorts)`
  计算；`NoSlvPorts=1` 时为 **0 位**（spec §5.1 公式的字面取值，tb_top C5.6），
  参考模型不得写死基线的 3 位。
