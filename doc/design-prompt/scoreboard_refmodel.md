...
- **C5.3** 事务数上限（依据 spec §2.1 `MaxMstTrans`/`MaxSlvTrans` 行 +
  §5.4.1/§5.4.2/§5.4.3、§0 行 2；`MaxSlvTrans` 侧机制为 §5.4.3 上游确认项）：
  ...
  - **每 slave 端口** ≤ `MaxMstTrans=10`（依据 spec §5.4.1）：M2-TL01 场景收窄到
    单一（低位 ID、方向、目标 master 端口）三元组构造（全程只用一个目标，避免与
    C5.1 的 stall 判据混淆）。**判决锚点须延迟不敏感（spec §7.4.5，BUG-0013 裁决）**：
    slave 端口侧 `SpillAw`/`SpillAr`（基线 `CUT_ALL_AX` 启用）位于分桶计数核心逻辑
    **之前**，外部 AW/AR 拒收边界的即时性是延迟不敏感表现——弹性缓冲只会使接受
    **更早**、不会更晚。故 checker 只准锚在延迟不敏感观测量上：判「**限内不假 stall**
    （该三元组在飞计数 ≤ `MaxMstTrans` 期间的边界 stall 仍是真失败）＋**上限最终
    被守**（在飞/完成记账口径下计数不越 `MaxMstTrans`）」；**不得**断言第 N+1
    （=第 11）笔在外部边界的某具体时点/拍数被拒收（弹性缓冲窗口深度随 `LatencyMode`
    变化、许可来源未给固定拍数，spec §7.4.5/§7.4.3）。
  - **每 master 端口每 ID**（前缀后完整 ID）≤ `MaxSlvTrans=6`（依据 spec §5.4.2）：
    REV-005 裁决已**解锁 M2-TL02 的弱化可观测上界 checker**——监视「每 master 端口
    × 每可观测（前缀后）ID × 每方向在飞事务数 ≤ `MaxSlvTrans`」，>6 即红。分组须
    按 `doc/bugs/BUG-0011.md` ## regression_guard 的不假红约束选取（**方向须分开
    计**，合池计数在"实为分向"机制读法下会假红；该分组在任何未决机制读法下都只是
    真实上界的保守下侧、绝不假红）。**机制级断言**（spec §5.4.3 列为上游确认项的
    精确拒收触发点/是否硬性反压）**不落地**——不预设"第几笔在某具体通道/时点被
    拒收"。
...
- **注**：M1 smoke 的 M1-02 激励构造为不制造 §5.2.1 的未决对（uvm_env C4.2 注），
  ...
  （M2-TL02 落地为已解锁的弱化可观测上界 checker，spec §5.4.2；机制级断言仍为
  §5.4.3 上游确认项、不落地）。
...
- M2 判据激活集：C1.5（地址表运行时版本化，M2-CFG01）、C5.1/C5.2/C5.5（保序 stall
  与时间戳基础设施，M2-OR01/OR02）、C5.3 slave 侧（事务上限，M2-TL01；master 侧
  受 spec §5.4.3 上游确认项约束、随 M2-TL02 弱化范围确认后落地）、C5.4（W 次序，
  M2-WO01）、C6.3 的原子读成对判据落地实际激励（M2-AT01）。C2（decode error）仍待 M3。
