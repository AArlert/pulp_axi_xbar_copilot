...
- **判决范围声明（BUG-0024，REV-011 §2.3 路线 (b)；实现约束详见 §4 C4.1）**：本条
  状态定义的"单槽模型"（"若该 key 同方向已有记录则不重复登记"）决定了本 SVA 的
  同 ID 保序判决范围为 **"每完整 ID 至多一笔在飞"**。spec §5.2.4 允许的同 ID 同向
  同目标 N≥2 合法堆积**超出本范围**——此时按接受序登记的序号会被最新一笔覆写、
  不再指向实际完成的最老一笔，若在此判决会假红（REV-011 §2.2 四步构造）。该 N≥2
  情形的完成序由 `scoreboard_refmodel.md` C5.1/C5.2 的**每事务队列判据**（按
  `accept_time` 逐笔入队）承担；本模块在 `w_reorder`/`r_reorder` 内对 N≥2 的完成
  **真正解除武装**（依模块自有在飞计数 `w_n[]`/`r_n[]` 早退，非仅注释）。
- cover：主属性前提发生 ≥1 次（激励来源 M2-OR01）；配套属性前提发生 ≥1 次（激励
  来源 M2-OR02）。范围边界见证 `w_lost_now`/`r_lost_now` 命中数随每次运行报出
  （REV-011 §2.3 b-3，激励来源 M3-OR05）。
- 适用端口：仅 slave 端口。
