...
|---|---|---|---|---|
| 1 | Does the chart archive know this? `grep -il "<assertion name / symptom>" doc/bugs/` | (reuse prior FL) | Read the old record first; test its rca hypothesis against your waveform before fresh analysis | orch puts the FL record into the dv card's inputs before any new tracing |
...
| 4 | Same seed, clean rebuild — does it still fire? | no → `TOOL_ENV` | Minimal repro; check `P-xxx` / TOOL_ENV FLs; record the environment cue | orch reruns (`make run`, clean rebuild); dv classifies |
| 5 | All above eliminated: locate `first_anomaly` in the waveform (`xdebug` backward trace from the assertion time) | `DUT_BUG` **candidate** | Write the FL with first_anomaly + chain, then request rev signoff — only rev's record makes it an asserted DUT bug | dv drafts the FL; rev must sign before "DUT bug" is claimed anywhere |
