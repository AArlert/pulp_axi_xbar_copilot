...
<!-- Project-owned since framework 0.8.0 (upstream retired .claude/skills/;
     kept here as a local asset — we maintain it). Consumer: before any ✅
     or CLOSED. -->
...
2. Scenario evidence:
   ...
   repro), then runs the docs check.
...
5. Replay and audit: invariant 2 guarantees line 1 of every record IS the
   replay command — run it as written (`make replay` retired in 0.8.0).
   `make check SCEN=<id>` prints the full trace (spec → evidence → bugs →
   reviews).
6. Milestone-level evidence stays human-made: regression summary copied
   ...
   checked by `make check MILESTONE=<n>`, which since 0.8.0 also requires at
   least one `KILL` row tagged to that milestone (invariant 5).
