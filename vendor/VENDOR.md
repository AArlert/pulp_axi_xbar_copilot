...
| P-002 | axi/src/axi_xbar.sv (行 64-67/84 module axi_xbar、行 182-187/190 module axi_xbar_intf) + axi/test/tb_axi_xbar_pkg.sv (行 29-33 class axi_xbar_monitor) | VCS-MX O-2018.09-SP2 无法在 elaboration 常量求值中将 `X - 1` 作打包维度位宽折叠（Error-[NCE] Non-constant expression），X 为 `xbar_cfg_t` struct 成员选择（`Cfg.NoAddrRules`，两处端口 `addr_map_i` 位宽）或同一参数列表内的兄弟参数（`NoAddrRules`，class 参数 `AddrMap` 位宽）。修法同 floo P-001 形态：在各 parameter/class 参数列表内新增 `localparam int unsigned AddrMapMsb = X - 1`，端口/参数宽度由 `[X-1:0]` 改用 `[AddrMapMsb:0]`（共 3 处宽度声明改写）。tb_axi_xbar_pkg.sv 一处属"先序错误掩盖"——axi_xbar.sv 两处修掉后同类 NCE 才浮出，按本卡"同一 P-002 记录同类 NCE"处置 | equivalent — `[AddrMapMsb:0]` 宽度 = AddrMapMsb+1 = (X-1)+1 = X，与原 `[X-1:0]` 宽度 X 逐位恒等；localparam 不可覆盖不进入可覆盖参数序，且三处例化（axi_xbar_intf→axi_xbar、tb→axi_xbar_monitor）均具名连接，插入 localparam 不改任何绑定；未动任何行为逻辑/端口连接/赋值语义 | BUG-0006 |  |
